...
module.exports = function(ss, options) {
  var loadFile;
  loadFile = function(dir, fileName, type, options, context, cb) {
    var extension, formatter, path;
    if (typeof context === 'function') {
      cb = context;
    }
    if (typeof cb === 'function' && context === void 0) {
      context = cb;
    }
    ...
    return formatter.compile(path.replace(/\\/g, '/'), options, context, cb);
  };
  return {
    ...
    html: function(path, opts, context, cb) {
      if (typeof context === 'function') {
        cb = context;
      }
      return loadFile(options.dirs.views, path, 'html', opts, context, cb);
    }
  };
};
